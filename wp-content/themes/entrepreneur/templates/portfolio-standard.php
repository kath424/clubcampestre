...
$args = array(
    ...
    if(isset($filter_bar[0]) && $filter_bar[0] == 1) {
        ...
        echo '<span>'. __('Sort:', 'ENTREPRENEUR') . '</span><a href="#" data-filter="*" class="current">'.__('All', 'ENTREPRENEUR').'</a>'. $project_type_links;
        echo '</div>';
    ...
    echo '<div id="portfolio-row" class="portfolio-row row '.$key .' '. $col_row_class.'">';
		if (!$loop->have_posts()) {
            ...
            _e('Sorry, no results were found.', 'ENTREPRENEUR');
            echo '</div>';
            get_search_form();
        }
    ...
    <div class="row">
		<?php if ($loop->max_num_pages > 1) : ?>
            <nav class="post-nav">
                <ul class="pager">
                    <li class="previous"><?php previous_posts_link(__('&larr; Previous', 'ENTREPRENEUR'), $loop->max_num_pages); ?></li>
                    <li class="next"><?php next_posts_link(__('Next &rarr;', 'ENTREPRENEUR'), $loop->max_num_pages); ?></li>
                </ul>
            </nav>
        <?php endif; ?>
	</div>
