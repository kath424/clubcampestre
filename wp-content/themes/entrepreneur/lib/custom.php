...
if ( ! function_exists( 'bittersweet_pagination' ) ) {
	function bittersweet_pagination()
	{
		...
		$pages = paginate_links(array(
			...
			'total' => $total,
			'type' => 'array',
			'prev_text' => esc_html__('Newer posts &rarr;', 'ENTREPRENEUR'),
			'next_text' => esc_html__('&larr; Older posts', 'ENTREPRENEUR'),
		));
		if (is_array($pages)) {
...
function themo_return_header_sidebar_settings($post_type = false) {
	...
	}elseif($post_type > "" && $post_type == "tribe_events"){ // Modern Tribe Events Calendar
        if(!is_single()){
            $key = "themo_events_index_layout";
            $show_header = ot_get_option( $key.'_show_header', "on" );
            $page_header_float = ot_get_option( $key.'_header_float', "centered" );
        }else{
            $key = "themo_single_event_layout";
            $show_header = ot_get_option( $key.'_show_header', "on" );
            $page_header_float = ot_get_option( $key.'_header_float', "centered" );
        }
        return array ($key, $show_header, $page_header_float,false);
...
function themo_custom_excerpt_more( $output ) {
	if ( (has_excerpt() || themo_has_more()) && ! is_attachment() && get_post_type() != 'themo_portfolio') {
		$output .= ' &hellip; <a href="' . get_permalink() . '">' . __('Read More', 'ENTREPRENEUR') . '</a>';
	}
	return $output;
}
...
function themo_read_more_link() {
	if (get_post_type() != 'themo_portfolio') {
		return ' &hellip; <a href="' . get_permalink() . '">' . __('Read More', 'ENTREPRENEUR') . '</a>';
	}
	...
	if ( function_exists( 'ot_get_option' ) ) {
		...
		if($favicon > ""){
			...
			if( $is_ping ){ // HTML output
				$favicon_html = "<link rel=\"shortcut icon\" href=\"".esc_url($favicon)."\" type=\"image/png\">\n";
				$favicon_html .= "<link rel=\"icon\" href=\"".esc_url($favicon)."\" type=\"image/png\">";
			}elseif ($is_ico){
				$favicon_html = "<link rel=\"shortcut icon\" href=\"".esc_url($favicon)."\" type=\"image/icon\">\n";
				$favicon_html .= "<link rel=\"icon\" href=\"".esc_url($favicon)."\" type=\"image/icon\">";
			}
		}else{
			$favicon_html = "";
		}
        ...
        $custom_color_css .= "html .woocommerce a.button.alt, html .woocommerce-page a.button.alt, html .woocommerce a.button, html .woocommerce-page a.button{background-color: $custom_color_primary !important; color: #fff;}";
        ...
        $custom_color_css .= "html .woocommerce a.button:hover,html .woocommerce-page a.button:hover{background-color: $custom_color_primary !important; color: #fff;}";
...
function themo_admin_meta_show()
{
	...
	// Admin Styles
	wp_register_style( 'themo_admin_css', get_template_directory_uri() . '/assets/css/admin-styles.css', false, '1.1' );
...
function themo_ot_meta_box_post_format_quote($array,$pages) {
	...
	$array['fields'] = array(
		...
		array(
			...
			'label'   => '',
			'desc'    => esc_html__( 'Source Name (ex. author, singer, actor)', 'ENTREPRENEUR' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
			...
			'label'   => '',
			'desc'    => esc_html__( 'Source Title (ex. book, song, movie)', 'ENTREPRENEUR' ),
			'std'     => '',
			'type'    => 'text'
		));
	return $array;
...
function themo_ot_meta_box_post_format_audio($array,$pages) {
	...
	$array['fields'] = array(
		array(
			...
			'desc'    => esc_html__( 'Use the built-in <code>[audio]</code> shortcode here.', 'ENTREPRENEUR' ),
			'std'     => '',
			'type'    => 'textarea'
		)
	);
...
function themo_ot_meta_box_post_format_link($array,$pages) {
	...
	$array['fields'] = array(
		...
		array(
			'id'      => '_format_link_url',
			'label'   => '',
			'desc'    => esc_html__( 'Link URL (ex. http://google.com)', 'ENTREPRENEUR' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
			...
			'label'   => '',
			'desc'    => esc_html__( 'Link Title (ex. Check out Google)', 'ENTREPRENEUR' ),
			'std'     => '',
			'type'    => 'text'
		),
		...
		array(
			...
			'label'       => esc_html__( 'Link Target', 'ENTREPRENEUR' ),
			'type'        => 'checkbox',
			'choices'     => array(
				array(
					'value'       => '_blank',
		...
		array(
			...
			'label'       => __( 'Link behaviour on the portfolio homepage', 'ENTREPRENEUR' ),
...
function themo_ot_meta_box_post_format_video($array,$pages) {
	...
	$array['fields'] = array(
		array(
			...
			'desc'    => sprintf( __( '(ex. http://vimeo.com/link-to-video). You can find a list of supported oEmbed sites in the %1$s.', 'ENTREPRENEUR' ), '<a href="http://codex.wordpress.org/Embeds" target="_blank">' . __( 'Wordpress Codex', 'ENTREPRENEUR' ) .'</a>' ),
			'std'     => '',
			'type'    => 'text'
		),
		array(
			...
			'desc'    => __( 'Use the built-in <code>[video]</code> shortcode here.', 'ENTREPRENEUR' ),
			'std'     => '',
			'type'    => 'textarea'
		)
	);
...
function themo_filter_ot_recognized_background_repeat( $array, $field_id ) {
	$array = array(
		'no-repeat' => esc_html__( 'No Repeat', 'ENTREPRENEUR' ),
		'repeat'    => esc_html__( 'Repeat All', 'ENTREPRENEUR' ),
		'repeat-x'  => esc_html__( 'Repeat Horizontally', 'ENTREPRENEUR' ),
		'repeat-y'  => esc_html__( 'Repeat Vertically', 'ENTREPRENEUR' ),
	);
...
function themo_filter_ot_recognized_background_attachment( $array, $field_id ) {
	$array = array(
		'fixed' => esc_html__( 'Fixed', 'ENTREPRENEUR' ),
		'scroll'    => esc_html__( 'Scroll', 'ENTREPRENEUR' ),
	);
...
function themo_register_required_plugins() {
	...
	$plugins = array(
		...
		array(
			'name'               => 'Entrepreneur Shortcodes', // The plugin name.
			'slug'               => 'entrepreneur-shortcodes', // The plugin slug (typically the folder name).
			'source'             => get_stylesheet_directory() . '/plugins/entrepreneur-shortcodes.zip', // The plugin source.
		...
		array(
			'name'               => 'Entrepreneur Custom Post Types', // The plugin name.
			'slug'               => 'entrepreneur-custom-post-types', // The plugin slug (typically the folder name).
			'source'             => get_stylesheet_directory() . '/plugins/entrepreneur-custom-post-types.zip', // The plugin source.
			'required'           => true, // If false, the plugin is only 'recommended' instead of required.
		...
		array(
			'name'               => 'Booked - Appointment Booking', // The plugin name.
			'slug'               => 'booked', // The plugin slug (typically the folder name).
			'source'             => get_stylesheet_directory() . '/plugins/booked.zip', // The plugin source.
			'required'           => false, // If false, the plugin is only 'recommended' instead of required.
			'version'            => '1.9.10', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
			'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
			'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
			'external_url'       => '', // If set, overrides default API URL and points to an external URL.
		),
        ...
        array(
            ...
            'required'           => false, // If false, the plugin is only 'recommended' instead of required.
            'version'            => '3.1.3', // E.g. 1.0.0. If set, the active plugin must be this version or higher.
            'force_activation'   => false, // If true, plugin is activated upon theme activation and cannot be deactivated until theme switch.
            'force_deactivation' => false, // If true, plugin is deactivated upon theme switch, useful for theme-specific plugins.
            'external_url'       => '', // If set, overrides default API URL and points to an external URL.
        ),
	...
	$config = array(
		...
		'strings'      => array(
			'page_title'                      => esc_html__( 'Install Required Plugins', 'ENTREPRENEUR' ),
			'menu_title'                      => esc_html__( 'Install Plugins', 'ENTREPRENEUR' ),
			'installing'                      => esc_html__( 'Installing Plugin: %s', 'ENTREPRENEUR' ), // %s = plugin name.
			'oops'                            => esc_html__( 'Something went wrong with the plugin API.', 'ENTREPRENEUR' ),
			'notice_can_install_required'     => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'ENTREPRENEUR' ), // %1$s = plugin name(s).
			'notice_can_install_recommended'  => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'ENTREPRENEUR' ), // %1$s = plugin name(s).
			'notice_cannot_install'           => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'ENTREPRENEUR' ), // %1$s = plugin name(s).
			'notice_can_activate_required'    => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'ENTREPRENEUR' ), // %1$s = plugin name(s).
			'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'ENTREPRENEUR' ), // %1$s = plugin name(s).
			'notice_cannot_activate'          => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'ENTREPRENEUR' ), // %1$s = plugin name(s).
			'notice_ask_to_update'            => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'ENTREPRENEUR' ), // %1$s = plugin name(s).
			'notice_cannot_update'            => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'ENTREPRENEUR' ), // %1$s = plugin name(s).
			'install_link'                    => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'ENTREPRENEUR' ),
			'activate_link'                   => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'ENTREPRENEUR' ),
			'return'                          => esc_html__( 'Return to Required Plugins Installer', 'ENTREPRENEUR' ),
			'plugin_activated'                => esc_html__( 'Plugin activated successfully.', 'ENTREPRENEUR' ),
			'complete'                        => esc_html__( 'All plugins installed and activated successfully. %s', 'ENTREPRENEUR' ), // %s = dashboard link.
...
function themo_import_end(){
	$meta_key_array = array('themo_service_block_1_groups','themo_testimonials_1_groups','themo_team_1_groups','themo_thumb_slider_1_groups',
		'themo_brands_1_groups','themo_slider_flex_groups','themo_portfolio_1_groups', 'themo_tour_1_groups','themo_pricing_plans_1_groups',
        'themo_service_block_split_1_groups','themo_showcase_1_groups','themo_featured_1_groups','themo_service_block_2_groups','themo_testimonials_2_groups',
        'themo_team_2_groups','themo_thumb_slider_2_groups','themo_brands_2_groups','themo_slider_flex_groups','themo_portfolio_2_groups',
        'themo_tour_2_groups','themo_pricing_plans_2_groups','themo_showcase_2_groups');

	// 1) Get Page ID by Title
	$page_names = array('Stylist Home','Trainer Home','Physician Home','Coach Home','Consultant Home','Contractor Home','Entrepreneur Home','Careers','Showcase','Barber Home'); // list of all possible page names.
	foreach ($page_names as $page_name) {
		$page = get_page_by_title( $page_name );
		if(!is_null($page)){ // Found something? bust out.
			break;
		}
	}

	//2) Get Term ID by name
	$term_names = array('stylist-home','trainer-home','physician-home','coach-home','consultant-home','contractor-home','entrepreneur-home','barber-home'); // list of all possible term names.
	foreach ($term_names as $term_name) {
		$term_id = term_exists( $term_name );
		if(!is_null($term_id)){ // Found something? bust out.
			break;
		}
	}
	//2b) Get Portfolio Term ID by name
	$port_term_names = array('barber-portfolio-home','stylist-portfolio-home','trainer-portfolio-home','physician-portfolio-home','coach-portfolio-home','consultant-portfolio-home','contractor-portfolio-home','entrepreneur-portfolio-home'); // list of all possible term names.
	foreach ($port_term_names as $port_term_name) {
		$port_term_id = term_exists( $port_term_name );
		if(!is_null($port_term_id)){ // Found something? bust out.
			break;
		}
	}

    //2c) Get the TERM ID (if exists) for service-block-split
    $service_block_split_term_names = array('service-block-split'); // list of all possible term names.
    foreach ($service_block_split_term_names as $service_block_split_term_name) {
        $service_block_split_term_id = term_exists( $service_block_split_term_name );
        if(!is_null($service_block_split_term_id)){ // Found something? bust out.
            break;
        }
    }

    //2d) Get the TERM ID (if exists) for testimonial-single
    $testimonial_sigle_term_names = array('testimonial-single'); // list of all possible term names.
    foreach ($testimonial_sigle_term_names as $testimonial_sigle_term_name) {
        $testimonial_sigle_term_id = term_exists( $testimonial_sigle_term_name );
        if(!is_null($testimonial_sigle_term_id)){ // Found something? bust out.
            break;
        }
    }

    //2e) Get the TERM ID (if exists) for service-block-double
    $service_block_double_term_names = array('service-block-double'); // list of all possible term names.
    foreach ($service_block_double_term_names as $service_block_double_term_name) {
        $service_block_double_term_id = term_exists( $service_block_double_term_name );
        if(!is_null($service_block_double_term_id)){ // Found something? bust out.
            break;
        }
    }


	//3) for each $meta_key_array - update_post_meta()
	if(isset($page->ID) && $page->ID > '' && $term_id > ''){
		foreach ($meta_key_array as $meta_key) {
			if($meta_key == 'themo_portfolio_1_groups' && $port_term_id > ''){
				update_post_meta($page->ID, $meta_key, $port_term_id);
			}elseif($meta_key == 'themo_service_block_split_1_groups' && $service_block_split_term_id > ''){ // Service Block Split
                update_post_meta($page->ID, $meta_key, $service_block_split_term_id);
            }elseif($meta_key == 'themo_testimonials_2_groups' && $testimonial_sigle_term_id > ''){ // Testimonial Single
                update_post_meta($page->ID, $meta_key, $testimonial_sigle_term_id);
            }elseif($meta_key == 'themo_service_block_2_groups' && $service_block_double_term_id > ''){ // service block double
                update_post_meta($page->ID, $meta_key, $service_block_double_term_id);
            }else{
				update_post_meta($page->ID, $meta_key, $term_id);
			}
		}
	}
	...
	// 4) Other Pages
	// 1) Get Page ID by Title
	$term_id = term_exists( 'entrepreneur-home' );
	$page_names = array('Entrepreneur Home','Tour','Tour 2','About','Careers','Contact 2','Contact Us Today','Showcase','Pricing'); // list of all possible page names.

	foreach ($page_names as $page_name) {
		$page = get_page_by_title( $page_name );
		if(!is_null($page) && !is_null($term_id)){ // Found something? bust out.
			if(isset($page->ID) && $page->ID > '' && $term_id > ''){
				foreach ($meta_key_array as $meta_key) {
					update_post_meta($page->ID, $meta_key, $term_id);
				}
	...
	// Tour 2 A
	$term_id = term_exists( 'tour-group-1' );
	$page = get_page_by_title('Tour');
	if(!is_null($page) && !is_null($term_id)){ // Found something? bust out.
		if(isset($page->ID) && $page->ID > '' && $term_id > ''){
			update_post_meta($page->ID, 'themo_tour_1_groups', $term_id);
		}
	}

	// Tour 2 B
	$term_id = term_exists( 'tour-group-2' );
	$page = get_page_by_title('Tour');
	if(!is_null($page) && !is_null($term_id)){ // Found something? bust out.
		if(isset($page->ID) && $page->ID > '' && $term_id > ''){
			update_post_meta($page->ID, 'themo_tour_2_groups', $term_id);
		}
	}

	// Careers
	$term_id = term_exists( 'careers' );
	$page = get_page_by_title('Careers');
	if(!is_null($page) && !is_null($term_id)){ // Found something? bust out.
		if(isset($page->ID) && $page->ID > '' && $term_id > ''){
			update_post_meta($page->ID, 'themo_accordion_1_groups', $term_id);
		}
	}

	// Locations
	$term_id = term_exists( 'locations' );
	$page = get_page_by_title('Locations');
	if(!is_null($page) && !is_null($term_id)){ // Found something? bust out.
		if(isset($page->ID) && $page->ID > '' && $term_id > ''){
			update_post_meta($page->ID, 'themo_service_block_split_1_groups', $term_id);
			update_post_meta($page->ID, 'themo_service_block_split_2_groups', $term_id);
			update_post_meta($page->ID, 'themo_service_block_split_3_groups', $term_id);
		}
	}

	/*echo '<div class="update-nag">';
	printf(__('<a href="%2$s">Back to Theme Activation 2</a> (not completed yet) | <a href="%1$s"><small><em>dismiss</em></small></a>', 'ENTREPRENEUR'), '?themo_activation_nag_ignore=0',admin_url('themes.php?page=theme_activation_options'));
	echo "</div>";*/
}
