...
;(function($, window, document, undefined) {
	...
	$win.on('load', function() {
		...
		function reset_add_timeslot_button(){
			...
			$('td.addTimeslot').find('select').prop('selectedIndex',0).show();
		...
		$('#bookedTimeslotsWrap').on('click','a.booked-add-timeslot',function(e){
			...
			if (thisButton.hasClass('button-primary')){
				...
				init_timeslot_tabs();
				var thisForm = $('#timepickerTemplate').prependTo(parentBlock).slideDown('fast');
...
function booked_datepicker_show_formatted_date( dateField, selectedDate ){
	...
	formatSpan.html( '<i class="fa fa-refresh fa-spin" aria-hidden="true"></i>' );
