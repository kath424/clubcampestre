...
function themo_shortcodes_stylesheet() {
    ...
    // Check for API Key, if available use it.
    if ( function_exists( 'ot_get_option' ) ) {
        if (defined('THEMO_MAP_API_KEY')) {
            $th_default_key = THEMO_MAP_API_KEY;
        } else{
            $th_default_key = false;
        }
        $themo_google_map_api_key = ot_get_option('themo_google_map_api_key', $th_default_key);
    }

    if($themo_google_map_api_key != false){
        // Register scripts
        wp_register_script( 'gpp_sc_googlemap_api', 'https://maps.googleapis.com/maps/api/js?sensor=false&key='.esc_attr($themo_google_map_api_key), array( 'jquery' ), '1.0.3', true );
    }else{
        // Register scripts
