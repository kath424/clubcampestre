...
function wpbm_up_link() {
	...
	return 'http://oplugins.com/plugins/booking-manager/' . ( ( empty($v) ) ? '' : 'upgrade-' . $v  . '/' ) ;
}
...
function wpbm_get_ssl_page_content( $url ) {
	...
	if ( 
		...
		debuge($error_message);																							//FixIn: 2.0.1.3
...
function wp_admin_bar_items_menu(){
	...
	if ( $update_count > 0 ) {
		$update_count_title = "&nbsp;<span id='ab-updates' class='wpbm-count bk-update-count' >" . number_format_i18n($update_count) . "</span>" ; //id='wpbm-count'
