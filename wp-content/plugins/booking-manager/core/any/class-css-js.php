...
abstract class WPBM_JS_CSS {
    ...
    function __construct() {        
        ...
        add_action( 'wpbm_load_js_on_admin_page',  array( $this, 'load_js_on_admin_page' ) );    // Load JS.   Hook fire only in admin pages of plugin. CLASS: WPBM_Admin_Menus (..\any\class\class-admin-menu.php)
        add_action( 'wpbm_load_css_on_admin_page', array( $this, 'load_css_on_admin_page' ) );   // Load CSS.  Hook fire only in admin pages of plugin. CLASS: WPBM_Admin_Menus (..\any\class\class-admin-menu.php)
    }
    ...
    public function load(){
        ...
        $is_load_scripts = apply_filters( 'wpbm_is_load_script_on_this_page', $is_load_scripts );
        ...
        if ( $this->getType() == 'css' ) 
            do_action( 'wpbm_enqueue_style',  ( is_admin() ? 'admin': 'client' ) );     
        
        if ( $this->getType() == 'js' ) 
            do_action( 'wpbm_enqueue_script', ( is_admin() ? 'admin': 'client' ) );     
    }
