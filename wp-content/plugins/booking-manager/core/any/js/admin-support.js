 ...
 * @returns {undefined}
 */
function wpbm_scroll_to( object_name ) {    
...
function wpbm_animate_border( element, time, colors, x ) {
    ...
    if (x >= colors.length) {
        ...
        setTimeout(function() {
            wpbm_animate_border( element, time, colors, x );
        }, time)
    }
}

function wpbm_field_highlight( object_name ) {    
    
    if ( jQuery( object_name ).length > 0 ) { 
     
        wpbm_scroll_to( object_name );
 ...
 * @returns {Boolean}
 */
function wpbm_are_you_sure( message_question ){
    var answer = confirm( message_question );
    if ( answer) { return true; }
    else         { return false;}
}

function wpbm_admin_show_message_processing( message_type ){
    
    var message = '' ;
    
    if ( message_type == 'saving' )
        message += wpbm_message_saving;
    else if ( message_type == 'updating' )
        message += wpbm_message_updating;
    else if ( message_type == 'deleting' )
        message += wpbm_message_deleting;
    else 
        message += wpbm_message_processing;
      
    if ( message == 'undefined' )  
        message = 'Processing'
      
    message = ' <span class="wpdevelop"><span class="glyphicon glyphicon-refresh wpbm_spin wpbm_ajax_icon"  aria-hidden="true"></span></span> ' + message + '...';
    
    wpbm_admin_show_message( message, 'info', 10000 );
}
 ...
 * @returns {undefined}
 */
function wpbm_admin_show_message( message, m_type, m_delay ){
    ...
    jQuery('#ajax_working').html(   '<div id="wpbm_alert_message" class="wpbm_alert_message">' +
                                        '<div class="wpbm_inner_message '+alert_class+'"> ' +
                                            '<a class="close" href="javascript:void(0)" onclick="javascript:jQuery(this).parent().hide();">&times;</a> ' + 
                                            message + 
                                        '</div>' +
                                    '</div>' 
                                );
    jQuery('#wpbm_alert_message').animate( {opacity: 1}, m_delay ).fadeOut(500);        
}


function wpbm_close_dropdown_selectbox( selector_id ) {
...
function wpbm_show_selected_in_dropdown( selector_id, title, value ){
...
// Exmaple: wpbm_show_selected_in_dropdown__radio_select_option( 'wh_ ... _date', 'wh_ ... _date2', 'wh_ ... _datedays_interval_Radios' );
function wpbm_show_selected_in_dropdown__radio_select_option( selector_id, selector_id2, radio_name ){
...
function wpbm_set_checkbox_in_table( el_stutus, el_class ){ 
 ...
 */
//<![CDATA[
function wpbm_verify_window_opening( us_id, window_id ){
        ...
        jQuery.ajax({                                           // Start Ajax Sending
                ...
                data:{
                    ...
                    window:     window_id,
                    is_closed:  is_closed,
                    wpbm_nonce: jQuery('#wpbm_admin_panel_nonce').val() 
                }
        });
 ...
 */
//<![CDATA[
function wpbm_save_custom_user_data( us_id, data_name, data_value , is_reload ){

        wpbm_admin_show_message_processing( 'saving' );   
...
// Contact Form
////////////////////////////////////////////////////////////////////////////////
function wpbm_submit_client_form( submit_form, wpdev_active_locale ){
    ...
    for (i=0; i<count; i++)   {
        ...
        if ( (element.type !=='button') && (element.type !=='hidden') ) {       
            ...
            /*if ( element.name == ('phone') ) {
                ...
                if ( inp_value != '' )
                    if(reg.test(inp_value) == false) {wpbm_show_error_message( element , message_verif_phone);return;}
            }*/


            // Validation Check -- Requred fields
            if ( element.className.indexOf( 'wpbm-validate-required' ) !== -1 ){      
                
                if  ( ( element.type =='checkbox' ) && ( element.checked === false ) ) {
                    if ( ! jQuery(':checkbox[name="'+element.name+'"]', submit_form).is(":checked") ) {
                        wpbm_show_error_message( element , wpbm_global1.message_verif_requred_for_check_box);
                        return;                            
                    }
                }
                if  ( element.type =='radio' ) {
                    if ( ! jQuery(':radio[name="'+element.name+'"]', submit_form).is(":checked") ) {
                        wpbm_show_error_message( element , wpbm_global1.message_verif_requred_for_radio_box);
                        return;                            
                    }
                }
                if  ( ( element.type !='checkbox' ) && ( element.type !='radio' ) && ( inp_value === '' ) ) {
                    wpbm_show_error_message( element , wpbm_global1.message_verif_requred);
                    return;
                }
            }

            // Validation Check --- Email correct filling field
            if ( element.className.indexOf( 'wpbm-validate-email' ) !== -1 ){                
                var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,})$/;
                if ( ( inp_value != '' ) && ( reg.test(inp_value) == false ) ) {
                    wpbm_show_error_message( element ,  wpbm_global1.message_verif_email );
                    return;
                }
            }
                ...
                if (primary_email_name != null) { // We found
                    ...
                    if (jQuery('[name="' + primary_email_name  + '"]').length > 0) {
                        ...
                        if ( jQuery('[name="' + primary_email_name  + '"]').val() !== inp_value ) {
                            wpbm_show_error_message( element , message_verif_same_emeil );return;
                        }
                    }
                }
 ...
 */
function wpbm_show_message_under_element( element , errorMessage , message_type) {
    
     wpbm_scroll_to( element );
    
     if ( jQuery( element ).attr('type') == "radio" ) {
        jQuery( element ).parent().parent().parent()
                .after('<span class="wpbm-near-field-message alert '+ message_type +'">'+ errorMessage +'</span>'); // Show message
    ...
    jQuery(".widget_wpbm .wpbm-near-field-message")
            .css( {'vertical-align': 'sub' } ) ;
    jQuery(".wpbm-near-field-message")
...
function wpbm_show_error_message( element , errorMessage) {
    ...
    if (jQuery("[name='"+ element.name +"']").attr('type') == "radio") {
        jQuery("[name='"+ element.name +"']").parent().parent()//.parent()
                .after('<span class="wpbm-near-field-message alert alert-warning">'+ errorMessage +'</span>'); // Show message
    ...
    if ( element.type == 'checkbox' )
        jQuery(".wpbm-near-field-message").css( { 'vertical-align': 'middle'} );
 ...
 * @returns {undefined}
 */
function wpbm_reload_page_with_paramater( url, param, value ) {
    var hash       = {};
...
jQuery(window).load(function(){
    ...
    ////////////////////////////////////////////////////////////////////////////
    if ( jQuery('.wpbm_colorpick').length > 0 ) {
        jQuery('.wpbm_colorpick').iris( {
            change: function(event, ui){
                jQuery(this).css( { backgroundColor: ui.color.toString() } );
            }
            , hide: true
        ...
        jQuery('.wpbm_colorpick').click(function(event){
            event.stopPropagation();
        });
    }
 ...
 */
function wpbm_reset_wp_editor_content( editor_textarea_id, editor_textarea_content ) {
